...
extension NSDate {
    ...
    private class func components(#fromDate: NSDate) -> NSDateComponents! {
        return Tools.defaultCalender().components(NSDate.componentFlags(), fromDate: fromDate)
    }
    ...
    func dateByAddingMonths(months: Int) -> NSDate
    {
        ...
        comp.month = months
        var calender:NSCalendar = Tools.defaultCalender()
        return calender.dateByAddingComponents(comp, toDate: self, options: nil)!
    ...
    func dateAtStartOfDay() -> NSDate
    {
        ...
        components.minute = 0
        components.second = 0
        return Tools.defaultCalender().dateFromComponents(components)!
    }
    ...
    func dateAtStartOfWeek() -> NSDate
    {
        ...
        var components = Tools.defaultCalender().components(flags, fromDate: self)
        components.weekday = 2 // monday
        components.hour = 0
        components.minute = 0
        components.second = 0
        
        if Tools.CompareDates(Tools.defaultCalender().dateFromComponents(components)!, date2: self) == DateComparison.Earlier{
            return Tools.defaultCalender().dateFromComponents(components)!.dateBySubtractingDays(7)
        }
    ...
    func monthDays () -> Int { return Tools.defaultCalender().rangeOfUnit(.DayCalendarUnit, inUnit: .MonthCalendarUnit, forDate: self).length }
    ...
    func isWeekend() -> Bool {
        let range = Tools.defaultCalender().maximumRangeOfUnit(.WeekdayCalendarUnit)
